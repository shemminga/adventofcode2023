...
long solve(char[][] grid) {
    ...
    var maxR = IntStream.range(0, h)
            .mapToLong(sI -> markEnergized(grid, sI, 0, Dir.R))
            .max()
            .orElseThrow();

    var maxL = IntStream.range(0, h)
            .mapToLong(sI -> markEnergized(grid, sI, w - 1, Dir.L))
            .max()
            .orElseThrow();

    var maxD = IntStream.range(0, w)
            .mapToLong(sJ -> markEnergized(grid, 0, sJ, Dir.D))
            .max()
            .orElseThrow();

    var maxU = IntStream.range(0, w)
            .mapToLong(sJ -> markEnergized(grid, h - 1, sJ, Dir.U))
            .max()
            .orElseThrow();

    return max(max(maxR, maxL), max(maxU, maxD)); // Mad Max.
}

long markEnergized(char[][] grid, int sI, int sJ, Dir sD) {
    var energized = new boolean[grid.length][grid[0].length];

    markEnergized(grid, energized, sI, sJ, sD);
...
void markEnergized(char[][] grid, boolean[][] energized, int sI, int sJ, Dir sD) {
    var q = new ArrayDeque<MovePoint>();
    var visited = new ArrayList<MovePoint>();
    q.addLast(new MovePoint(sI, sJ, sD));
