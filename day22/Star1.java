...
long solve(Brick[] bricks) {
    ...
    if (test) {
        for (Brick brick : bricks) {
            out.println("brick = " + brick);
        }
    }

    return countDisintegratables(bricks);
}

long countDisintegratables(Brick[] bricks) {
    var disintegratables = 0L;
    for (int i = 0; i < bricks.length; i++) { // Brick to disintegrate
        var disintegratable = true;
        for (int j = 0; j < bricks.length; j++) { // Brick to test if it drops after disintegration of i
            if (i == j) continue;
            var down = bricks[j].moveDown();

            if (!down.intersects(bricks[i])) // Doesn't rest on brick i, so is fine
                continue;

            var restsOnOther = false;
            for (int k = 0; k < bricks.length; k++) {
                if (j == k) continue;
                if (i == k) continue;

                if (down.intersects(bricks[k])) {
                    restsOnOther = true;
                    break;
                }
            }
        ...
        if (disintegratable) disintegratables++;
    }
